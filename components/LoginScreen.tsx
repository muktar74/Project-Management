...
const BankLogo = ({ className }: { className?: string }) => (
    <div className={`flex items-center justify-center text-white ${className}`}>
        ...
        <h1 className="ml-3 text-4xl font-bold tracking-wider">
...
const LoginScreen: React.FC<LoginScreenProps> = ({ users, onLogin }) => {
  ...
  const handleLoginAttempt = (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      // For this demo, any password is valid.
      onLogin(user);
    } else {
  ...
  return (
    <div className="min-h-screen flex bg-neutral-50">
      {/* Left Branding Panel */}
      <div className="hidden lg:flex w-1/2 bg-brand-primary text-white flex-col justify-between p-12">
        <BankLogo className="self-start"/>
        <div className="text-center">
            <h2 className="text-4xl font-bold leading-tight">Project Management and Technical Support Office Hub</h2>
            <p className="mt-4 text-lg text-neutral-300 max-w-sm mx-auto">Your central command for tracking, managing, and delivering projects with precision The first full-fledged interest-free bank in Ethiopia.</p>
        </div>
        <p className="text-center text-sm text-neutral-400">&copy; 2025 Zamzam Bank. All rights reserved.</p>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md">
            <div className="lg:hidden mb-10 text-center">
                <h1 className="text-3xl font-bold text-brand-primary">ZamzamBank PMO</h1>
            </div>
